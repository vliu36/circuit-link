...
export async function deleteUserAccount() {
    ...
    if (user) {
        try {
            ...
            const res = await fetch(`${process.env.SERVER_URI}/api/users/delete/${user.uid}`, {
                method: "DELETE",
                headers: {
            ...
            window.location.href = "http://${process.env.CLIENT_URI}/signin"; // Redirect to sign-in page
        } catch (error: any) {
